...
function parseAmliWebsite(html, url) {
  ...
  $(".floorplan-card").each((i, element) => {
    try {
      ...
      const unitName =
        $(element).find(".floorplan-card__name").text().trim() || "Unknown";
      ...
      const priceText =
        $(element).find(".floorplan-card__price").text().trim() || "0";
      ...
      const floorInfo =
        $(element).find(".floorplan-card__floor").text().trim() || "";
  ...
  $(selector).each((i, element) => {
    try {
      ...
      const bedroomEl = $(element).find(
        "[class*='bedroom'], [class*='bed-'], .beds, .bedrooms"
      );
      ...
      const bathroomEl = $(element).find(
        "[class*='bathroom'], [class*='bath-'], .baths, .bathrooms"
      );
      ...
      const priceEl = $(element).find(
        "[class*='price'], [class*='rent'], .price, .rent"
      );
      ...
      const floorEl = $(element).find(
        "[class*='floor'], [class*='level'], .floor, .level"
      );
