...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!knownUnits) {
      ...
      console.log(`Loaded ${knownUnits.size} known units`);
    }

    console.log("Starting property check...");
    ...
    for (const result of results) {
      summary += `• ${result.property || result.propertyName}: Found ${
        result.matchingUnits.length
      } matching units total\n`;
    }
